...
class Proxy
  def initialize(target_object)
    ...
    # ADD MORE CODE HERE
    @message_call_history_array = Array.new
  end

  # WRITE CODE HERE
  def method_missing(method_name, *args, &block)
    if @object.respond_to?(method_name)
      @message_call_history_array << method_name
  ...
  def messages
    @message_call_history_array
  end

  def called?(method)
    @message_call_history_array.include?(method)
  end

  def number_of_times_called(method)
    @message_call_history_array.count(method)
  end
end
